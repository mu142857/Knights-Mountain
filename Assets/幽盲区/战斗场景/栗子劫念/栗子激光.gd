...
func appear():
	$Line2D.modulate = Color(1.4, 1.4, 1.4)
	var tween = get_tree().create_tween()
	tween.parallel().tween_property($Line2D, "width", 15, 0.5)
	tween.parallel().tween_property($Line2D, "modulate", Color(3.6, 1.6, 1.6), 0.15)

func disappear():
	var tween = get_tree().create_tween()
	tween.parallel().tween_property($Line2D, "width", 0, 0.05)
	tween.parallel().tween_property($Line2D, "modulate", Color(1, 1, 1), 0.05)
