...
func _ready() -> void:
	...
	if barrage_position <= 3:
		flight_time = 1.5
		the_gravity = 2500
		$Summon.start(1)
	elif barrage_position <= 7:
		flight_time = 1.5
		the_gravity = 1000
		$Summon.start(1)
...
func _physics_process(delta: float) -> void:
	...
	if self.global_scale.x < 1.5:
		self.global_scale += Vector2(0.05, 0.05)
	else:
		self.global_scale = Vector2(1.5, 1.5)
	
	if $AnimatedSprite2D.animation != "Move":
		return
	
	self.z_index = 1 # 全飞弹通用部分
	if self.global_position.y >= 810 and $AnimatedSprite2D.animation != "Stop":
		
		if barrage_position >= 4:
			Game.flash(0.2, Color(3.0, 0.6, 0.58))
			var gyph = preload("res://Assets/幽盲区/战斗场景/幽居提琴哥/低音谱号落地特效.tscn").instantiate()
...
func _on_animated_sprite_2d_animation_finished() -> void:
	...
	if $AnimatedSprite2D.animation == "Ready":
		...
		if player and barrage_position <= 7:
			...
			pass
		elif player and barrage_position == 15:
			target_pos = player.global_position + Vector2(position_change, 0)
		else:
